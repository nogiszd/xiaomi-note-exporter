  ...
  <Card>
    <CardHeader class="space-y-3">
      ...
      <p class="text-sm text-muted-foreground">
        {{ exportStore.current }} / {{ exportStore.total || "?" }} notes, {{ exportStore.imagesCount }} images, elapsed:
        {{ elapsedSeconds }}s
      </p>
    </CardHeader>
    ...
    <CardContent class="space-y-3">
      ...
      <ScrollArea class="h-52 rounded-md border p-3">
        <ul class="space-y-1 text-sm">
          <li v-for="(line, index) in exportStore.logs" :key="`${index}-${line}`" class="text-muted-foreground">
            {{ line }}
          </li>
        </ul>
      </ScrollArea>
    ...
    <CardFooter>
      <Button v-if="exportStore.isRunning" type="button" variant="destructive" @click="exportStore.cancel()">
        Cancel
      </Button>
      <Button v-else type="button" variant="outline" @click="openFolder">Open Folder</Button>
    </CardFooter>
  </Card>
</template>
